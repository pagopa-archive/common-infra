...
#tfsec:ignore:azure-storage-queue-services-logging-enabled reason: queue not used in this storage
resource "azurerm_storage_account" "grafana_storage" {
  name                     = replace("${local.project}grafanadatast", "-", "")
  location                 = var.location
...
#tfsec:ignore:azure-appservice-authentication-enabled reason: authentication managed by application
#tfsec:ignore:azure-appservice-require-client-cert    reason: authentication managed by application, no client-cert required
module "grafana_service" {
  ...
  always_on         = true
  linux_fx_version  = "DOCKER|grafana/grafana-oss:8.2.6"
